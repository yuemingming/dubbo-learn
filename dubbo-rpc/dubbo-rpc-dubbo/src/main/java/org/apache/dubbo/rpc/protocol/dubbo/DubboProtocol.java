...
public class DubboProtocol extends AbstractProtocol {
    ...
    /**
     * 通过实现ExchangeHandlerAdapter，实现处理request的请求，当请求来时，将invocation转发到对应的Invoker对象
     */
    ...
    private void openServer(URL url) {
        ...
        if (isServer) {
            ...
            if (server == null) {
                ...
                // server supports reset, use together with override 通信服务器已存在，重置服务器属性
                server.reset(url);
            }
        }
    }
    ...
    public <T> Invoker<T> refer(Class<T> serviceType, URL url) throws RpcException {
        ...
        //获得远程通信数组
        /**
         *        创建DubboInvoker对象{@link #getClients(URL)}
         */
        // create rpc invoker.
    ...
    private ExchangeClient initClient(URL url) {
        ...
        //设置编码器为Dubbo，即DUbboCountCodec
        url = url.addParameter(Constants.CODEC_KEY, DubboCodec.NAME);
        // enable heartbeat by default 默认开启heartbeat
        url = url.addParameterIfAbsent(Constants.HEARTBEAT_KEY, String.valueOf(Constants.DEFAULT_HEARTBEAT));
