...
/**
 * javassist是一个开源的分析、编辑和创建java字节码的类库。通过使用javassist对字节码操作可以实现动态“AOP”
 * 框架。关于java字节码处理，目前有很多工具，如bcel,asm（cglib只是对asm又封装了一层）。不过这些都需要直接跟虚拟机指令
 * 打交道。javassist的主要优点，在于简单而且快速，直接使用java编码的形式，而不需要了解虚拟机指令，就能动态改变类的结构，
 * 或者动态生成类。
 */
    ...
    public Class<?> doCompile(String name, String source) throws Throwable {
        ...
        if (matcher.find()) {
            ...
            if (extend.contains(".")) {//内嵌的类，例如：extends A.B
                extendClass = extend;
            } else if (fullNames.containsKey(extend)) {//引用指定的类
        ...
        if (matcher.find()) {
            ...
            for (String iface : ifaces) {
                iface = iface.trim();
                String ifaceClass;
                if (iface.contains(".")) {//内嵌接口，例如：implements A.B
                    ifaceClass = iface;
        ...
        //匹配method。使用分割的方式，实际上，分割出来的不仅仅有方法。
