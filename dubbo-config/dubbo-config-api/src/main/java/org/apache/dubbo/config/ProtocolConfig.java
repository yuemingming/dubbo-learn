 ...
 * ProtocolConfig
 * 服务提供者协议配置
 * 属性参数参见：https://dubbo.gitbooks.io/dubbo-user-book/references/xml/dubbo-protocol.html
 * @export
 */
public class ProtocolConfig extends AbstractConfig {
    ...
    // thread pool
    private String threadpool;//线程池类型，可选fixed/cached

    // thread pool core thread size
    private Integer corethreads;

    // thread pool size (fixed size)
    private Integer threads;//服务线程池大小（固定大小）

    // IO thread pool size (fixed size)
    private Integer iothreads;//io线程池大小（固定大小）

    // thread pool's queue length
    private Integer queues;//线程池队列大小。

    // max acceptable connections
    private Integer accepts;//服务提供方可接受最大连接数

    // protocol codec
    private String codec;//协议编码方式

    // serialization
    private String serialization;//协议序列化方式 dubbo,hessian2,java,compactedjava,json
    ...
    // buffer size
    private Integer buffer;//网络读写缓冲区大小

    // heartbeat interval
    private Integer heartbeat;//心跳间隔

    // access log
    private String accesslog;//设为true，将向loggger中输出访问日志，也可 填写访问日志文件路径，直接把访问日志输出到指定文件

    // transfort
    private String transporter;//协议的服务端和客户端的实现类型
    ...
    // whether to register
    private Boolean register;//是否注册到注册中心
