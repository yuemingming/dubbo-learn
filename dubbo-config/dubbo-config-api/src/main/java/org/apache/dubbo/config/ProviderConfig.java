 ...
 * 同时该标签为<dubbo:service>和<dubbo:protocol>标签的缺省值配置
 * @export
...
public class ProviderConfig extends AbstractServiceConfig {
    ...
    private String host;//服务主机名，自动查找本机IP
    ...
    private Integer threads;//服务线程池大小

    // IO thread pool size (fixed size)
    private Integer iothreads;//IO线程池大小

    // thread pool queue length
    private Integer queues;//线程等待队列

    // max acceptable connections
    private Integer accepts;//最大可接受连接数
